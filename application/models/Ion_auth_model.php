...
//            $this->console_log("Bcrypy", true);
//            $this->console_log($this->bcrypt->hash($password), true);
            return $this->bcrypt->hash($password);
        }


        if ($this->store_salt && $salt) {
//            $this->console_log("SHA", true);
//            $this->console_log(sha1($password . $salt));
            return sha1($password . $salt);
        } else {
            $salt = $this->salt();
//            $this->console_log("SHA2", true);
     ...
     * @author Mathew
     **/
    public function change_password_custom($id, $old, $new)
    {
        $query = $this->db->select('id, password, salt')
            ->where("id", $id)
            ->limit(1)
        ...
        if ($query->num_rows() !== 1) {
            ...
            if ($old_password_matches === TRUE) {
                ...
                if ($successfully_changed_password_in_db) {
                    return $this->common->getGenericResponse("response", null, "Password Updated");
                } else {
                    return $this->common->getGenericErrorResponse(501, 'Unable to update password');
                }
            } else {
                return $this->common->getGenericErrorResponse(501, 'Old Password not valid');
            }
        }
    }
...
//	Testing
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        if ($this->is_max_login_attempts_exceeded($identity)) {
            ...
            $this->hash_password($password); //Testing
    ...
    public function getUsersCustom($id = NULL)
    {
        $this->trigger_events('user');
